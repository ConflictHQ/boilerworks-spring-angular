...
public class WorkflowService {
  ...
  public List<WorkflowDefinitionDto> findAllDefinitions() {
    return definitionRepository.findAll().stream().map(WorkflowDefinitionDto::from).toList();
  }
  ...
  public List<TransitionLogDto> getTransitionHistory(UUID instanceId) {
    return transitionLogRepository
        .findByWorkflowInstanceIdOrderByPerformedAtAsc(instanceId)
        .stream()
        .map(TransitionLogDto::from)
        .toList();
  }
